# 改自原版https://github.com/sibears/IDAGolangHelper，对Go 1.16二进制文件处理时报错的改动
...
class TypeProcessing(object):
    ...
    def handle_offset(self, offset):
        ...
        print("==> offset: " + hex(offset) + "  ==> getname: " + name) ###################################
        idc.set_cmt(offset, name, 0)

        #get kind name
        kind_name = self.getKindEnumName(offset)
        print("= get kind_name: ", kind_name)
    ...
    def betterTypePlease(self, offset):
        print("[debug] got to Types.py:407 betterTypePlease")#####################
        kind_name = self.getKindEnumName(offset)
        if kind_name in self.kind_types:
            print("== Types.py:409 betterTypePlease")
    ...
    def makePtrType(self, offset):
        print("[debug] got to makePtrType")
    ...
    def makeStructType(self, offset):
        print("[debug] got to makeStructType...")
        print("[debug] offset: ", hex(offset))
        ...
        name = Utils.relaxName(name)
        name = "ut_" + name
        print("[debug] call self.createUserTypeStruct 参数\n==> ",(addr, name, size, size_new_struct))
    ...
    def createUserTypeStruct(self, addr, name, size, self_size):
        print("[debug] got to createUserTypeStruct...")
        fields = []
        sid = ida_struct.get_struc_id("structField")
        ...
        for i in range(size):
            ...
            print("[debug] type_addr: ", hex(type_addr))
            typename = self.getType(type_addr)
            print("[debug] typename: ", typename)
    ...
    def makeInterface(self, offset):
        print("[debug] got to makeInterface...")
...
class TypeProcessing17(TypeProcessing):
    ...
    def getName(self, offset):
        ...
        string_addr = self.getOffset(name_off) + 2
        print("[debug] string_addr: ", hex(string_addr))
        ln = idc.get_wide_byte(string_addr-1)
        return self.get_str(string_addr, ln)

    def nameFromOffset(self, offset):
        addr = offset
        print("[debug] nameFromOffset, addr: ", hex(addr))
        return self.get_str(addr + 2, idc.get_wide_byte(addr + 1))
    ...
    def processIMethod(self, offst):
        ...
        name = self.get_str(name + 2, idc.get_wide_byte(name + 1))
        return name
